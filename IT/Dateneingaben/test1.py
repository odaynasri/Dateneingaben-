...
# === Excel-Export ===
def exportiere_als_excel():
    conn = sqlite3.connect(db_datei)
    df = pd.read_sql_query("SELECT * FROM benutzer", conn)
    dateiname = "benutzer_export.xlsx"
    df.to_excel(dateiname, index=False, engine='openpyxl')
    conn.close()
    print(f"Daten wurden in {dateiname} exportiert.")

# === Buttons ===
tk.Button(window, text="Speichern", command=speichern).pack(pady=10)
tk.Button(window, text="Als Excel exportieren", command=exportiere_als_excel).pack(pady=5)
